...
glm::vec3 BasicTracer::SampleCosineHemisphere(JobData& jobData, const glm::vec3& normal) noexcept
{
...
glm::vec3 BasicTracer::SampleGGXDirection(
...
float BasicTracer::ComputeSpecularProbability(const glm::vec3& viewDir, const glm::vec3& normal,
...
glm::vec3 BasicTracer::GenerateRandomDirection(JobData& jobData) noexcept
{
...
glm::vec3 BasicTracer::ApplyColorCorrection(const JobData& jobData, const glm::vec3& color) noexcept
{
...
void BasicTracer::RenderBlock(const Block& block)
{
    ...
    auto renderPixelBuffered = [&](int x, int y)
    {
        sampleBuffer.clear();
        ...
        const float averageIntensity =
            std::accumulate(sampleBuffer.begin(), sampleBuffer.end(), 0.0f,
        ...
        const float standardDeviation =
            std::accumulate(sampleBuffer.begin(), sampleBuffer.end(), 0.0f,
                ...
                {
                    const float diff = (colorToIntensity(color) - averageIntensity);
    ...
    auto renderPixelCummulative = [&](int x, int y)
    {
        sampleBuffer.clear();
        const auto baseU = static_cast<float>(x) * pixelSize.x;
        const auto baseV = static_cast<float>(y) * pixelSize.y;
    ...
    for (int y = block.offset.y; y < block.offset.y + block.size.y; ++y)
    {
        for (int x = block.offset.x; x < block.offset.x + block.size.x; ++x)
        {
            if (useStatisticFireflyElimination)
                renderPixelBuffered(x, y);
            else
                renderPixelCummulative(x, y);
        }
    }
